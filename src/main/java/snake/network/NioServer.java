...
public abstract class NioServer implements IServer {
  ...
  protected void closeSession(ISession session) {
    if (Config.DEBUG_MESSAGE_LOGGING) {
      logger.debug("Closing session: " + session.getSessionId());
    }
    if (session instanceof NioSession) {
      try {
  ...
  private void handleRead(SelectionKey key) {
    ...
    try {
      ...
      logger.warn(
          "IOException in handleRead for " + session.getSessionId() + ": " + e.getMessage());
      closeSession(session);
    } catch (Exception e) {
      logger.error("Error parsing message from " + session.getSessionId() + ": " + e.getMessage());
      closeSession(session);
    }
  }
